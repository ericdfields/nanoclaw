  ...
  async connect(): Promise<void> {
    ...
    this.bot.on('message:photo', async (ctx) => {
      ...
      try {
        ...
        const filename = `tg-${ctx.message.message_id}-${Date.now()}.jpg`;
        const processed = await processImage(buffer, groupDir, filename);
        imagePath = processed.filePath;

        logger.info(
          { chatJid, sender: senderName, size: buffer.length },
          'Telegram photo downloaded and processed',
        );
      } catch (err) {
